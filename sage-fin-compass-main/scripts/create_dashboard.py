...
dashboard_path = 'c:/Users/urvas/OneDrive/Desktop/CodeCrafters/CodeCrafter_Mango/sage-fin-compass-main/src/components/CompanyDashboard.tsx'

dashboard_code = """import { useState, useMemo } from "react";
import { TrendingUp, TrendingDown, FileText, Download, BarChart3, Clock, X, CheckCircle, Sparkles, ExternalLink, BookmarkPlus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { useWatchlist } from "@/hooks/useWatchlist";
import { toast } from "sonner";
import { XAxis, YAxis, CartesianGrid, ResponsiveContainer, Tooltip, BarChart, Bar } from "recharts";
import { NewsItem, CompanyStock } from "@/data/dummyData";
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass-card p-3 shadow-xl border-border/50 text-xs text-foreground z-50 relative bg-background">
...
interface Props {
  mainStock: CompanyStock;
  filteredResults: NewsItem[];
  hideRightColumn?: boolean;
  onRelatedCompanyClick?: (c: string) => void;
  matchingStocks?: CompanyStock[];
}

export default function CompanyDashboard({ mainStock, filteredResults, hideRightColumn, onRelatedCompanyClick, matchingStocks = [] }: Props) {
  ...
  return (
    <>
      <div className={`grid gap-8 ${hideRightColumn ? 'lg:grid-cols-1' : 'lg:grid-cols-3'} animate-in fade-in slide-in-from-bottom-4 duration-500`}>
        
        {/* Left Column (or full width if hidden) */}
        <div className={`${hideRightColumn ? 'col-span-1' : 'lg:col-span-2'} space-y-8`}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-4 rounded-xl border bg-background/50 hover:bg-card hover:shadow-sm transition-all group">
                  ...
                  <Button onClick={(e) => { e.stopPropagation(); handleDownloadReport(report.type, report.quarter, mainStock.symbol); }} variant="ghost" size="icon" className="group-hover:text-primary group-hover:bg-primary/10 transition-colors">
          ...
          <div>
            ...
            <div className="space-y-4">
              {filteredResults.slice(0, 3).map((item) => (
                <div key={item.id} onClick={(e) => { e.stopPropagation(); setSelectedNews(item); }} className="cursor-pointer glass-card p-5 shadow-sm transition-all hover:shadow-md hover:border-primary/50">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <a onClick={(e) => e.stopPropagation()} href={item.sourceUrl} className="text-xs text-primary font-medium hover:underline flex items-center gap-1">
        ...
        {!hideRightColumn && (
          <div className="space-y-6">
            ...
            {matchingStocks && matchingStocks.length > 1 && (
              <div className="glass-card p-6">
                ...
                <div className="space-y-3">
                  {matchingStocks.slice(1, 5).map(s => (
                    <div key={s.symbol} onClick={() => onRelatedCompanyClick?.(s.symbol)} className="flex justify-between items-center p-3 hover:bg-background/80 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-border">
                      <div>
      ...
      {/* Selected News Overlay */}
      {selectedNews && (
        <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-sm animate-in fade-in flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedNews.relatedCompanies.map(c => (
                      <Badge key={c} variant="outline" className="cursor-pointer hover:bg-primary hover:text-primary-foreground transition-colors" onClick={() => { setSelectedNews(null); onRelatedCompanyClick?.(c); }}>
                        {c}
                      </Badge>
                    ))}
                  </div>
...
print("Created CompanyDashboard.tsx!")
